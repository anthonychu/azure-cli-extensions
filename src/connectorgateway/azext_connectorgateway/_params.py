...
def load_arguments(self, _):
    ...
    for scope in [
            'connectorgateway',
            'connectorgateway available-connector',
            'connectorgateway connection',
            'connectorgateway connection access-policy',
            'connectorgateway mcp-server-config',
            'connectorgateway trigger-config']:
        with self.argument_context(scope) as c:
            c.argument('resource_group_name', arg_type=resource_group_name_type)

    for scope in [
            'connectorgateway create',
            'connectorgateway show',
            'connectorgateway update',
            'connectorgateway delete']:
        with self.argument_context(scope) as c:
            c.argument('name', name_type)

    with self.argument_context('connectorgateway create') as c:
    ...
    with self.argument_context('connectorgateway update') as c:
        c.argument('tags', tags_type)
    ...
    with self.argument_context('connectorgateway available-connector show') as c:
    ...
    for scope in [
            'connectorgateway connection create',
            'connectorgateway connection show',
            'connectorgateway connection update',
            'connectorgateway connection delete',
            'connectorgateway connection list-consent-links',
            'connectorgateway connection authorize']:
        with self.argument_context(scope) as c:
            c.argument('name', name_type)

    for scope in ['connectorgateway connection create', 'connectorgateway connection update']:
        with self.argument_context(scope) as c:
    ...
    with self.argument_context('connectorgateway connection list-consent-links') as c:
    ...
    with self.argument_context('connectorgateway connection authorize') as c:
        c.argument('redirect_url', options_list=['--redirect-url'], default='https://portal.azure.com',
                   help='OAuth redirect URL used when requesting consent links.')
    ...
    for scope in [
            'connectorgateway connection access-policy create',
            'connectorgateway connection access-policy show',
            'connectorgateway connection access-policy update',
            'connectorgateway connection access-policy delete']:
        with self.argument_context(scope) as c:
            c.argument('name', name_type)

    with self.argument_context('connectorgateway connection access-policy') as c:
    ...
    for scope in [
            'connectorgateway mcp-server-config create',
            'connectorgateway mcp-server-config show',
            'connectorgateway mcp-server-config update',
            'connectorgateway mcp-server-config delete']:
        with self.argument_context(scope) as c:
            c.argument('name', name_type)
    ...
    for scope in [
            'connectorgateway trigger-config create',
            'connectorgateway trigger-config show',
            'connectorgateway trigger-config update',
            'connectorgateway trigger-config delete']:
        with self.argument_context(scope) as c:
            c.argument('name', name_type)
